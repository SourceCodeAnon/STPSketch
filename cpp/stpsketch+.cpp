...
static vector<vector<int>> g_decay_lut;
static int  g_lut_N          = 0;
static bool g_lut_initialized = false;

void init_decay_lut(int recent_window_count) {
    if (g_lut_initialized && g_lut_N == recent_window_count) {

        return;
    }

    g_lut_N = recent_window_count;
    g_lut_initialized = true;

    g_decay_lut.assign(g_lut_N + 1, vector<int>(g_lut_N + 1, 0));

    for (int delta = 0; delta <= g_lut_N; ++delta) {
        for (int cnt = 0; cnt <= g_lut_N; ++cnt) {
            if (cnt == 0) {
                g_decay_lut[delta][cnt] = 0;
                continue;
            }

            double factor = static_cast<double>(delta) / g_lut_N;
            int decay_count = static_cast<int>(factor * cnt);
            int new_cnt = cnt - decay_count;
            if (new_cnt < 0) new_cnt = 0;
            g_decay_lut[delta][cnt] = new_cnt;
        }
    }
}

inline int decay_lookup(int delta, int counter, int recent_window_count) {
    if (!g_lut_initialized || g_decay_lut.empty()) {

        throw std::runtime_error("decay_lookup called before init_decay_lut");
    }

    if (delta < 0) delta = 0;
    if (delta > g_lut_N) delta = g_lut_N;
    if (counter < 0) counter = 0;

    if (counter > g_lut_N) throw std::runtime_error("decay_lookup: counter exceeds recent_window_count");

    return g_decay_lut[delta][counter];
}
...
public:
    ...
    void decay(int current_time_window) {
        ...
        counter = decay_lookup(delta, counter, recent_window_count);
    }
...
public:
    ...
    Bucket_Cell(uint32_t f_, int window_count_)
        : f(f_), window_count(window_count_), flags(0ULL), flag_sum(0) {}
...
public:
    ...
    Persistent_Part(int window_count_, double mem_MB, uint32_t hash_seed_, int bucket_cell_count_)
        ...
        int freq_bits = static_cast<int>(
            std::ceil(std::log2(static_cast<double>(window_count) + 1.0))
        );
        int cell_mem_bit = window_count + 32 + freq_bits;
    ...
    void insert(uint32_t f) {
        ...
        if (res.inserted) {
            ...
            if (cm_res > min_val && cell->query_flag(idx) != 1) {
                ...
                CM_part.set_new_value(cell->f, cell->query());

                cell->f = f;
                int count = 0;
                int n = cm_res - min_val;
                for (int i = 0; i < window_count; ++i) {
                    int bit_idx = (current_time_window + i) % window_count;
                    if (cell->query_flag(bit_idx) == 0) {
                        cell->set_1(bit_idx);
                        count++;
                        if (count == n) break;
                    }
...
AllTimeSlices load_time_slices(const string &base_dir) {
    ...
    const size_t MAX_PACKETS = 20000000ULL;
    const size_t WINDOW_SIZE = 40000ULL;
...
int main() {
    ...
    string base_dir = "/mnt/c/Users/45512/Documents/dataset/caida_2019/5min/ip_num/";
        ...
        for (double total_mem_MB : mem_lst_MB) {
            for (int CM_row_num : row_num_list) {
                for (int bucket_cell_count : bucket_cell_counts) {
                    ...
                    long long lut_bits = 1LL * (window_count + 1) * (window_count + 1) * CM_counter_size;
                    double lut_mem_MB = static_cast<double>(lut_bits) / (1024.0 * 1024.0 * 8.0);
                    double effective_total_mem_MB = total_mem_MB - lut_mem_MB;
                    cout << "LUT memory (MB): " << lut_mem_MB << '\n';
                    total_mem_MB = effective_total_mem_MB;
                    cout << "Effective total memory (MB): " << total_mem_MB << '\n';

                    if (total_mem_MB <= 0) {
                        throw std::runtime_error("effective_total_mem_MB <= 0, LUT 占用超过总预算");
                    }
